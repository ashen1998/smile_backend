...
public class GameServiceImpl implements GameService {
    ...
    @Override
    public Long setScore(Long id, Long score) {
    ...
    @Override
    public List<ScoreDto> getAllUsersSortedByScore() {
        List<User> all = userRepository.findAll(Sort.by(Sort.Direction.DESC, "score"));
        return all.stream().map(this::formatUserEntity).toList();
    }

    private ScoreDto formatUserEntity(User user) {
        return modelMapper.map(user, ScoreDto.class);
    }
}
